...
def excel_Processor(dateFrom, dateTo):
    ...
    if len(myError) == 0:
        ...
        for i in range(len(OCRs)):
            newWorksheet.cell(row=i + 1, column=1).value = OCRs[i]
            newWorksheet.cell(row=i + 1, column=2).value = sum[i]
    ...
    if isReady == True:
        with open("example.xlsx", "rb") as file:
            myBase64.append(base64.b64encode(file.read()).decode("utf-8"))

        excelMergerTool(
            myBase64, ["OCR_STNK", "OCR_KTP", "OCR_NPWP", "OCR_BPKB", "OCR_KK", "SUM"]
        )
